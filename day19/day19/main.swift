...
//  day19
//
//  Created by Jerry Hsu on 12/19/21.
//
...
struct Point: Equatable, Hashable {
    ...
    func rotate(major: Int, minor: Int, amount: Int) -> (Int, Int) {
        switch amount {
            case 0: return (major, minor)
            case 90: return (minor, -major)
            case 180: return (-major, -minor)
            case 270: return (-minor, major)
            default:
                fatalError("rotation is multiple of 90")
        }
    }
    
    func apply(rotation: Rotation) -> Point {
        switch rotation.axis {
            case .x:
                let (y, z) = rotate(major: self.y, minor: self.z, amount: rotation.amount)
                return Point(x: x, y: y, z: z)
            case .y:
                let (x, z) = rotate(major: self.x, minor: self.z, amount: rotation.amount)
                return Point(x: x, y: y, z: z)
            case .z:
                let (x, y) = rotate(major: self.x, minor: self.y, amount: rotation.amount)
                return Point(x: x, y: y, z: z)
        }
    }
}
...
struct Rotation {
    ...
    static func x(_ amount: Int) -> Rotation {
        return Rotation(axis: .x, amount: amount)
    }

    static func y(_ amount: Int) -> Rotation {
        return Rotation(axis: .y, amount: amount)
    }

    static func z(_ amount: Int) -> Rotation {
        return Rotation(axis: .z, amount: amount)
    }
}
...
struct Orientation {
    ...
    static let allOrientations: [Orientation] = [
        Orientation(rotations: []),
        Orientation(rotations: [.x(90)]),
        Orientation(rotations: [.x(180)]),
        Orientation(rotations: [.x(270)]),
        Orientation(rotations: [.y(180)]),
        Orientation(rotations: [.y(180), .x(90)]),
        Orientation(rotations: [.y(180), .x(180)]),
        Orientation(rotations: [.y(180), .x(270)]),
        Orientation(rotations: [.y(90)]),
        Orientation(rotations: [.y(90), .z(90)]),
        Orientation(rotations: [.y(90), .z(180)]),
        Orientation(rotations: [.y(90), .z(270)]),
        Orientation(rotations: [.y(270)]),
        Orientation(rotations: [.y(270), .z(90)]),
        Orientation(rotations: [.y(270), .z(180)]),
        Orientation(rotations: [.y(270), .z(270)]),
        Orientation(rotations: [.z(90)]),
        Orientation(rotations: [.z(90), .y(90)]),
        Orientation(rotations: [.z(90), .y(180)]),
        Orientation(rotations: [.z(90), .y(270)]),
        Orientation(rotations: [.z(270)]),
        Orientation(rotations: [.z(270), .y(90)]),
        Orientation(rotations: [.z(270), .y(180)]),
        Orientation(rotations: [.z(270), .y(270)])
    ]
}
...
struct Scanner {
    ...
    func adjustedPoints() -> [Point] {
        return points.map { (point) in
            orientation.rotations.reduce(point, { partialResult, rotation in
                partialResult.apply(rotation: rotation)
            })
        }
        .map { (point) in
            Point(x: point.x + position.x, y: point.y + position.y, z: point.z + position.z)
        }
    }
    
    func points(in orientation: Orientation) -> [Point] {
        return points.map { (point) in
            orientation.rotations.reduce(point, { partialResult, rotation in
                partialResult.apply(rotation: rotation)
            })
    ...
    init(lines: String, name: Int) {
        self.name = name
        points = Set(lines.components(separatedBy: "\n").dropFirst()
            .map {
...
func parse(input: String) -> [Scanner] {
    return input.components(separatedBy: "\n\n").enumerated()
        .map { (index, line) in
            Scanner(lines: line, name: index)            
        }
}

// If other scanner has matches, then return a verison of other scanner
// with the proper orientation and position.
func hasCommonBeacons(points: Set<Point>, with other: Scanner) -> Scanner? {
    for point in points {
        for orientation in Orientation.allOrientations {
            let otherPoints = other.points(in: orientation)
            for otherPoint in otherPoints {
                let dx = point.x - otherPoint.x
                let dy = point.y - otherPoint.y
                let dz = point.z - otherPoint.z
                let otherNormalized = Set(otherPoints.map { Point(x: $0.x + dx, y: $0.y + dy, z: $0.z + dz)})
                if points.intersection(otherNormalized).count >= 12 {
                    var result = other
