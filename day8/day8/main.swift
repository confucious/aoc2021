...
//  day8
//
//  Created by Jerry Hsu on 12/8/21.
//
...
enum Digit {
    static let zero: Set<Segment> = [.a, .b, .c, .e, .f, .g]      // 6
    static let one: Set<Segment> = [.c, .f]                       // 2
    static let two: Set<Segment> = [.a, .c, .d, .e, .g]           // 5
    static let three: Set<Segment> = [.a, .c, .d, .f, .g]         // 5
    static let four: Set<Segment> = [.b, .c, .d, .f]              // 4
    static let five: Set<Segment> = [.a, .b, .d, .f, .g]          // 5
    static let six: Set<Segment> = [.a, .b, .d, .e, .f, .g]       // 6
    static let seven: Set<Segment> = [.a, .c, .f]                 // 3
    static let eight: Set<Segment> = [.a, .b, .c, .d, .e, .f, .g] // 7
    static let nine: Set<Segment> = [.a, .b, .c, .d, .f, .g]      // 6
}

struct Input {
    let inputDigits: [Set<Segment>]
    let outputDigits: [Set<Segment>]

    init(input: String) {
        let components = input.components(separatedBy: " | ")
        self.inputDigits = components[0].split(separator: " ")
            .map { digit in
                Set(digit.map { Segment(rawValue: String($0))! })
            }
        self.outputDigits = components[1].split(separator: " ")
            .map { digit in
                Set(digit.map { Segment(rawValue: String($0))! })
            }
    }
...
func parse(input: String) -> [Input] {
    return input.split(separator: "\n")
        .map { Input(input: String($0)) }
}

func countUniques(data: [Input]) {
    var count = 0
    for line in data {
        for digit in line.outputDigits {
            if digit.count == 2
                || digit.count == 3
                || digit.count == 4
                || digit.count == 7 {
...
func decodeMapping(_ input: Input) -> Int {
    let twoSegment = input.inputDigits.first { $0.count == 2 }!
    let threeSegment = input.inputDigits.first { $0.count == 3}!
    let fourSegment = input.inputDigits.first { $0.count == 4 }!
//    let fiveSegments = input.inputDigits.filter { $0.count == 5 }
    let sixSegments = input.inputDigits.filter { $0.count == 6 }
    let sevenSegments = input.inputDigits.first { $0.count == 7 }!
    let a = threeSegment.subtracting(twoSegment).first!
    let maybeCorF = twoSegment
    let maybeBorD = fourSegment.subtracting(maybeCorF)
    let nine = sixSegments.first { $0.isSuperset(of: maybeCorF) && $0.isSuperset(of: maybeBorD) }!
    let six = sixSegments.first { $0.isSuperset(of: maybeBorD) && !$0.isSuperset(of: maybeCorF) }!
    let zero = sixSegments.first { $0.isSuperset(of: maybeCorF) && !$0.isSuperset(of: maybeBorD) }!
    let e = sevenSegments.subtracting(nine).first!
    let c = sevenSegments.subtracting(six).first!
    let d = sevenSegments.subtracting(zero).first!
    let f = maybeCorF.subtracting([c]).first!
    let b = maybeBorD.subtracting([d]).first!
    let g = nine.subtracting(fourSegment).subtracting([a]).first!
    
    let segmentMap: [Segment:Segment] = [
        a: .a,
        b: .b,
        c: .c,
        d: .d,
    ...
    var result = 0
    for digit in input.outputDigits {
        let decoded: Set<Segment> = Set(digit.map { segmentMap[$0]! })
        result *= 10
        switch decoded {
        case Digit.zero: result += 0
        case Digit.one: result += 1
        case Digit.two: result += 2
        case Digit.three: result += 3
        case Digit.four: result += 4
        case Digit.five: result += 5
        case Digit.six: result += 6
        case Digit.seven: result += 7
        case Digit.eight: result += 8
        case Digit.nine: result += 9
        default:
            fatalError("Couldn't match \(decoded)")
        }
    }
    return result
}
...
func process(data: [Input]) {
    var result = 0
    for line in data {
        let lineResult = decodeMapping(line)
        print("\(line.outputDigits) : \(lineResult)")
        result += lineResult
    }
    print(result)
}

//countUniques(data: parse(input: data))
//process(data: parse(input: sample1))
process(data: parse(input: data))
